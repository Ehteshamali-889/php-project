...
<div class="container-fluide text-black  bg-info" >
 ...
 <?php 
  $id = $_SESSION['userCnic'];
  $sql = "select * from user where cnic = '$id'";
  $result = mysqli_query($connect, $sql) or die("Error is : ". mysqli_error($connect)); 
  $row = mysqli_fetch_assoc($result);
 ?>

 <div class="container ">
   <div class="row  "><h2 class="text-centre text-white">Welcome <?php echo $_SESSION['userName'];?></h2></div>
    ...
    <div class="row border">
        <div class="col-2 border text-white"><?php echo $row["user_name"]?></div>
        <div class="col-2 border text-white"><?php echo $row["email"]?></div>
        <div class="col-2 border text-white"><?php echo $row["phone_number"]?></div>
                        ...
                        <div class="row">
                          <div class="offset-3"></div>
                            ...
                            <?php 
                             $myDate = date("y-m-d");
                             $userId = $row["user_id"];
                             $sql = "select * from appointment WHERE  appointment_date = '$myDate' ";
                             $query = mysqli_query($connect, $sql) or die("Error is : ". mysqli_error($connect));
                             
                             while( $result = mysqli_fetch_assoc($query))
                             {
                               $sp_id = $result["sp"];
                                $newSql = "select * from service_provider where
                                 sp_id = ' $sp_id '";
                               $spQuery = mysqli_query($connect, $newSql)  or die("Error is : ". mysqli_error($connect));
                               $spResult = mysqli_fetch_assoc($spQuery);
                              ?>
                              <div class="row">
                                <div class="col-lg-12 text-white border border-white mt-2 mb-2">
                                  <h5> Your Appointment Was on 
                                    <?php echo  $result["appointment_date"];?>
                                    with <?php echo $spResult["sp_name"] ?>
                                  </h5>
                                </div>
                              </div>
                             <?php
                            ...
                            <!-- Ider kaam karna h query lgani h  -->
                            <div class="offset-3"></div> 
                        </div>
                      </div>
                  </div>
                        ...
                        <div class="row">
                          <div class="offset-3"></div>
                             ...
                             while( $result = mysqli_fetch_assoc($query))
                             {
                              ...
                              <div class="row">
                                <div class="col-lg-12 text-white border border-white mt-2 mb-2">
                                  <h5> Your Upcoming Appointment is on 
                                    <?php echo  $result["appointment_date"];?>
                                    with <?php echo $spResult["sp_name"] ?>
                                  </h5>
                                </div>
